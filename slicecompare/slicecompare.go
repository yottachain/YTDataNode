...
//	TaskHandler         SliceCompareHandler
}
...
func (sc *SliceComparer)SaveRecordToTmpDB(hashBatch [][]byte, tmpFileName string) error {
	DBPath := util.GetYTFSPath() + tmpFileName
	log.Println("tmpDB:",DBPath)
	db,err := leveldb.OpenFile(DBPath,nil)
	defer db.Close()
	nowtime := strconv.FormatInt(time.Now().Unix(),10)
    for _, key := range hashBatch{
		nowtime = strconv.FormatInt(time.Now().Unix(),10)
		fmt.Println("dn_hash:",string(key))
...
func (sc *SliceComparer)SaveSnRecordToDB(hashBatch [][]byte, firstEntryIdx string, fileSnDBName string, firstEntryIdxFile string) error {
	...
	for value, key := range hashBatch{
		...
		log.Println("sn_hash:",string(key))
...
func (sc *SliceComparer)GetValueFromFile(fileName string) (string ,error){
	filePath := util.GetYTFSPath() + fileName
	log.Printf("getvaluefromfile: filepath=%s\r\n", filePath)
...
/*
func GetIndexForDownloadFromSN() int64 {
	firstIdxLastDownld := GetValueFromFile(FileFirstEntryIdx)
	comparedEntryLastDownld := GetValueFromFile(FileComparedIdx)

	return (firstIdxLastDownld + comparedEntryLastDownld - 1)
}
*/


func (sc *SliceComparer)CompareEntryWithSnTables(snHashBatch [][]byte, tmpHashDB, comparedIdxFileName string) error {
	...
	log.Printf("CompareEntryWithSnTables: filepath=%s\r\n", filePath)
...
//	iter := sn_db.NewIterator(nil, nil)
	for _,key := range snHashBatch {
		//key := iter.Key()
		log.Println("CompareEntryWithSnTables key:",key)
		if ok,err := tmp_db.Has(key,nil); ok == true{
			...
			log.Printf("[Fatal error] key=%s saved in supernode, but not found in datanode!!",key)
			return err
		}
		f.Truncate(0)
		f.WriteAt(key,0)
...
func (sc *SliceComparer)SaveEntryInDBToDel(tmpHashDB, toDelEntryDB string) error {
	var saveTime string
