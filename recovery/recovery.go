...
func NewEngine(ytfs *ytfs.YTFS, selfNode ythost.Host, opt *DataRecoverOptions) (*DataRecoverEngine, error) {
...
func (recoverEngine *DataRecoverEngine) RecoverData(td *TaskDescription) bool {
...
func (recoverEngine *DataRecoverEngine) RecoverStatus(td *TaskDescription) TaskResponse {
	...
	if status, ok := recoverEngine.taskStatus[td.ID]; ok {
		// free entry if it's done
		if status.Status == SuccessTask || status.Status == ErrorTask {
			delete(recoverEngine.taskStatus, td.ID)
		}
		return status
	}
	return TaskResponse{UnknownTask, UnknownTask.String()}
}
...
func (recoverEngine *DataRecoverEngine) retrieveData(loc P2PLocation, hash common.Hash) ([]byte, error) {
	// Read p2p network
	data, _ := recoverEngine.p2phelper.RetrieveData(loc, hash[:])
	return data, nil
}
