...
func VerifyVHF(data []byte, vhf []byte) bool {
	sha := crypto.MD5.New()
	sha.Write(data)
	return bytes.Equal(sha.Sum(nil), vhf)
}

func CaculateHash(data []byte)  []byte{
	sha := crypto.MD5.New()
	sha.Write(data)
	return sha.Sum(nil)
}
...
func (req *UploadShardRequest) GetResponseToClientByCode(code int32, seq uint64, privkey string) ([]byte, error) {
